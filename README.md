...
Koremiteは、貼り付けた文字起こしテキストを送りやすく・見返しやすいまとめに整えるiPhoneアプリです。

MVPの使い方はシンプルです。ボイスメモや別のアプリで文字起こしされたテキストをコピーし、Koremiteに貼り付け、保存先フォルダと重視ポイントを選んでまとめを生成、コピー/共有/保存する。

## できること
- 貼り付けた文字起こしテキストを受け付ける
- LINEやメッセージで送れる「短くまとめ」を生成する
- 見返しやすい「詳しくまとめ」を生成する
- 「元の記録」（話者別エントリ）を保持する
- 生成済みまとめをローカルに保存する
...
このページは古いMacでもUXを確認するためのPreviewで、保存済みまとめとフォルダはブラウザの`localStorage`に保存する。同じブラウザ内では再読み込み後も残るが、端末間同期はしない。
